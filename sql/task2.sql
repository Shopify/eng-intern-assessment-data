...
SELECT pr.product_id, pr.product_name, pr.avg_rating
FROM (
    SELECT p.product_id AS product_id, p.product_name, AVG(r.rating) AS avg_rating
...
FROM 
    ConOrders
WHERE 
    DATEDIFF(order_date, prev_order_date) = 1
GROUP BY 
    user_id, username;
