...
SELECT * FROM Products INNER JOIN Categories ON Products.category_id = Categories.category_id WHERE category_name="Sports & Outdoors";
...
SELECT DISTINCT Users.user_id, username, COUNT(Orders.order_id) FROM Users LEFT JOIN Orders ON Users.user_id = Orders.user_id GROUP BY Users.user_id;
...
SELECT Products.product_id, product_name, AVG(rating) FROM Products LEFT JOIN Reviews ON Products.product_id = Reviews.product_id GROUP BY Products.product_id;
...
SELECT Users.user_id, username, SUM(total_amount) as grand_total_spent FROM Users LEFT JOIN Orders ON Users.user_id = Orders.user_id GROUP BY Users.user_id ORDER BY grand_total_spent DESC LIMIT 5;
