...
-- Problem 9 Solution
SELECT TOP 3 T1.category_id, T1.category_name, SUM(T1.sales) as total_sales_amount
FROM 
(SELECT T.category_id, T.category_name, O.quantity * O.unit_price AS sales
FROM 
...
SELECT DISTINCT T2.user_id, U.username
FROM
(SELECT T1.product_id, T1.order_id, O.user_id
FROM 
...
-- Problem 11 Solution
SELECT DISTINCT T.product_id, T.product_name, T.category_id, T.max_price
FROM
(SELECT C.category_id, P.product_id, P.product_name, P.price, MAX(P.price) as max_price
FROM Categories C LEFT JOIN Products P
ON C.category_id = P.category_id
