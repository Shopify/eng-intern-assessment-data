...
-- Hint: You may need to use subqueries, joins, and aggregate functions to solve this problem.
 ...
 -- Selecting the category ID, category name, and the sum of sales amount.
SELECT c.category_id, c.category_name, SUM(oi.quantity * oi.unit_price) AS total_sales
FROM Categories c
JOIN Products p ON c.category_id = p.category_id
JOIN Order_Items oi ON p.product_id = oi.product_id
GROUP BY c.category_id, c.category_name
ORDER BY total_sales DESC -- Ordering the results by total sales in descending order to find the top categories.
...
-- Selecting the user ID and username.
SELECT u.user_id, u.username
FROM Users u
JOIN Orders o ON u.user_id = o.user_id -- link users to their orders.
JOIN Order_Items oi ON o.order_id = oi.order_id --finding out the category of each product.
...
GROUP BY u.user_id
-- The subquery in the HAVING clause calculates the total number of products in the "Toys & Games" category.
...
-- Selecting the product ID, product name, category ID, and price from a ranked subquery.
...
FROM (
    ...
    SELECT p.product_id, p.product_name, p.category_id, p.price,
           RANK() OVER (PARTITION BY p.category_id ORDER BY p.price DESC) AS price_rank -- Ranks the products within each category based on their price in descending order using the RANK() function
    FROM Products p
) AS ranked_products
...
-- Selecting user ID and username from the Users table.
-- Use a subquery to get each order's previous and next order dates for each user using LAG and LEAD function
-- DATEDIFF function calculates the difference in days between consecutive orders.
...
WHERE 
    -- Checking for orders on 3 consecutive days.
